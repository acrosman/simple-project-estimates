...
describe('buildHistogramPreview', () => {
  ...
  test('Returns early when min is negative', () => {
    ...
    const result = buildHistogramPreview(mockTargetNode, list, -1, 4, 'Test');
    expect(result).toBeUndefined();
  });

  test('Returns early when max is less than min', () => {
    const list = [0, 1, 2, 3, 4];
    const result = buildHistogramPreview(mockTargetNode, list, 10, 5, 'Test');
    expect(result).toBeUndefined();
  });

  test('Returns early when histogram has no data', () => {
    const list = [0, 0, 0, 0, 0]; // All zeros, yMax will be 0
    const result = buildHistogramPreview(mockTargetNode, list, 0, 4, 'Test');
    expect(result).toBeUndefined();
  });
  ...
  test('Calculates bucket count correctly for small ranges', () => {
    ...
    list[5] = 10;
    const result = buildHistogramPreview(mockTargetNode, list, 0, 9, 'Hours');
    expect(result).toBeUndefined();
  });
